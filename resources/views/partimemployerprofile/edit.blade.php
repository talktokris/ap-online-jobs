    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card auth-form">
                ...
                <div class="card-body">
                <form method="POST" action="{{route('partimemployerprofile.update',$employer->user_id)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="form-group row">
                            ...
                            ?>
                            <div class="col-sm-6">
                                <label for="name">{{ __('Select tasks that you want to get done:') }}<span class="text-danger">*</span></label><br>
                                <select placeholder="Name" class="form-control" id="task" name="service_task[]" multiple="multiple">
...
<script>
    ...
    (function () {
        $.get('/json-states?states_id=' + {{$employer->state}} , function(data){
            console.log(data);
            var city_id={{$employer->city}}
            $('#person_city').empty();
            $('#person_city').append('<option value="" disable="true" selected="true">-------  Select City -------</option>');

            $.each(data,function(index, statesObj){
                if(city_id==statesObj.id){
                    $('#person_city').append('<option value="'+statesObj.id +'" selected>'+ statesObj.name +'</option>');
                }else{
                    $('#person_city').append('<option value="'+statesObj.id +'">'+ statesObj.name +'</option>');
                }
            });
